  ...
  <my-col class="wrap">
    <my-col class="wrap-fir">
      <my-card class="card">
        ...
        <div class="btn-box">
          <div :class="regionClass[item.unite_id] ? 'btn on' : 'btn'" @click="changeRegion(item.unite_id)" v-for="(item, i) in $store.getters['overseas_common_realTime/getList1'].parent" :key="i">
            {{item.area_app_name}}
          </div>
        </div>
      </my-card>
...
export default {
  ...
  , watch: {
    regionArr() {
      var list = this.$store.getters['overseas_common_realTime/getList1']
