...
import inlineChart from 'src/components/online-chart'
export default {
  name: 'online-data',
    ...
    overviewList() {
      ...
      result.forEach(res => {
        ...
        this.onlineData.forEach((item, index) => {
          ...
          res.xAxis.push(item["统计时间"])
          res.tableData.push({date:item["统计时间"],data:Number(item[res.name])})
          // res.xAxis.push(item[res])
