  ...
  <div>
    ...
    <my-row class="selection-box">
      <div class="banner">
        ...
        <section style="margin-left:20px">
          <el-select
            v-model="gameListIndex"
            @change="changeGame(gameListIndex)"
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <span
              v-show="activityIndex != scope.$index"
            >{{$store.getters['o_c_divided_into/getTableData'][scope.$index][item]}}</span>
          </template>
        </el-table-column>
        <el-table-column
  ...
  methods: {
    ...
    changeRow(index) {
      if (!this.$store.getters['o_c_divided_into/getTableData'][index]['分成比例']) {
        return Utils.Notification.warning({ message: '请将分成比例填写完整' })
      }
      if (confirm("确认修改？")) {
        var params = {
